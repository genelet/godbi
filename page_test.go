...
func TestPage(t *testing.T) {
	...
	schema := NewSchema(db, map[string]Navigate{"ta":ta, "tb":tb})
	...
	lists, err = schema.Run("ta", METHODS["LIST"], args)
	if err != nil { panic(err) }
...
// [map[id:1 tb_topics:[map[child:john id:1 tid:1] map[child:sam id:1 tid:2]] x:a1234567 y:b1234567 z:zzzzz]]
	...
	if lists, err = schema.Run("tb", METHODS["DELETE"], map[string]interface{}{}, extra); err != nil { panic(err) }
	if lists, err = schema.Run("ta", METHODS["DELETE"], map[string]interface{}{}, extra); err != nil { panic(err) }

...
// [map[id:1 tb_topics:[map[child:john id:1 tid:1] map[child:sam id:1 tid:2]] x:a1234567 y:b1234567 z:zzzzz]]
	...
	if lists, err = graph.RunContext(ctx, "tb", METHODS["DELETE"], map[string]interface{}{"tid":1}, extra); err != nil { panic(err) }
	if lists, err = graph.RunContext(ctx, "tb", METHODS["DELETE"], map[string]interface{}{"tid":2}, extra); err != nil { panic(err) }
	if lists, err = graph.RunContext(ctx, "ta", METHODS["DELETE"], map[string]interface{}{"id":1}); err != nil { panic(err) }
